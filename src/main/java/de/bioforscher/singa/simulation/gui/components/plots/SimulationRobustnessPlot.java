...
import de.bioforscher.singa.simulation.gui.components.panes.ModuleOverviewPane;
...
public class SimulationRobustnessPlot extends LineChart<Number, Number> {
    ...
    private Data<Number, Number> indicator;
    ...
    private void configureIndicator() {
        Series<Number, Number> indicatorSeries = new Series<>();
        this.indicator = new Data<>();
    ...
    private void configureThreshold() {
        ...
        while (isInRange) {
            ...
            if (distance.getValue().doubleValue() < 1000) {
                // add data
                thresholdSeries.getData().add(new Data<>(distance.getValue(), timeStepCounter));
                // add time
                timeStepCounter += +1;
            } else {
                isInRange = false;
